...
function isSnmpError(iface) {
  return iface.ifAdminStatus === 0 && iface.ifOperStatus === 0;
}
...
export default function PortList() {
  ...
  const fetchNotInUse = () => {
    axios
      .get("http://localhost:5000/api/interfacesnotinuse")
      .then((res) => {
        ...
        .then((res) => {
          ...
          filteredData.forEach((iface) => {
            ...
            if (
              ...
              axios
                .post("http://localhost:5000/api/port-logs", {
  ...
  const notInUseKeys = new Set(
    Array.isArray(notInUseInterfaces)
      ? notInUseInterfaces.map((n) => `${n.device_id}_${n.ifIndex}`)
      : [],
  );
  ...
  const downPorts = interfaces.filter(
    (i) =>
      ...
      !isSnmpError(i) &&
      searchFilter(i) &&
      !notInUseKeys.has(`${i.device_id}_${i.ifIndex}`),
  );
  ...
  const notInUseFiltered = (
    Array.isArray(notInUseInterfaces) ? notInUseInterfaces : []
  ).filter(searchFilter);
  ...
  const handleCheckboxChange = (item, checked) => {
    if (checked) {
      ...
      setSelectedInterfaces((prev) =>
        prev.filter(
          (i) =>
            `${i.device_id}_${i.ifIndex}` !==
            `${item.device_id}_${item.ifIndex}`,
        ),
      );
    }
  };
  ...
  const handleRemove = (id) => {
    if (
      window.confirm(
        "Are you sure you want to remove this interface from not in use?",
      )
    ) {
      axios
        .delete(`http://localhost:5000/api/interfacesnotinuse/${id}`)
        .then(() => {
  ...
  return (
    ...
    >
      ...
      >
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <Button
            ...
            sx={{
              bgcolor: "rgb(141, 35, 15) !important",
              color: "#fff",
      ...
      <Box sx={{ display: "flex", flexWrap: "wrap", gap: 2 }}>
        {tabValue === 0 &&
          downPorts.map((i) => (
            <PortBlock
              ...
              item={i}
              showCheckbox
              isSelected={selectedInterfaces.some(
                (sel) =>
                  `${sel.device_id}_${sel.ifIndex}` ===
                  `${i.device_id}_${i.ifIndex}`,
              )}
        ...
        {tabValue === 3 &&
          notInUseFiltered.map((i) => (
            <PortBlock
              key={i.id}
              item={i}
              showMinus
              onMinusClick={() => handleRemove(i.id)}
            />
          ))}
      </Box>
  ...
  return (
    ...
    >
      ...
      >
        ...
        {showMinus && (
          <IconButton size="small" onClick={onMinusClick}>
            <RemoveIcon />
          </IconButton>
        )}
      </Box>
