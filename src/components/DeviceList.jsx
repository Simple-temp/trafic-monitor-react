...
import { 
  ...
  Box, Typography, Tabs, Tab, IconButton, Paper
} from "@mui/material";
...
const DeviceList = () => {
  ...
  const [formData, setFormData] = useState({ 
    hostname: "", ip_address: "", snmp_community: "", options: "" 
  });
  ...
  const handleSaveDevice = async () => {
    try {
      ...
      setOpenDialog(false);
      setEditingId(null);
      setFormData({ hostname: "", ip_address: "", snmp_community: "", options: "" });
      fetchDevices();
  ...
  const filteredDevices = devices.filter((d) => {
    ...
    const matchesStatus = tabValue === 0 ? d.status !== "UP" : d.status === "UP";
    ...
    <div style={styles.container}>
      <header style={styles.header}>
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Typography variant="h5" fontWeight="900" color="#334155">NMS Devices</Typography>
          <Box display="flex" gap={2}>
            <TextField size="small" placeholder="Search IP/Name..." value={globalSearch} onChange={(e) => setGlobalSearch(e.target.value)} sx={{bgcolor: '#fff', borderRadius: 1}} />
            <Select size="small" value={filterZone} onChange={(e) => setFilterZone(e.target.value)} displayEmpty sx={{bgcolor: '#fff', minWidth: 120, borderRadius: 1}}>
            ...
            <Button variant="contained" onClick={() => { setEditingId(null); setOpenDialog(true); }} sx={{bgcolor: '#b50000', '&:hover': {bgcolor: '#8b0000'}}}>Add Device</Button>
          </Box>
        </Box>
      </header>

      <Tabs value={tabValue} onChange={(e, v) => setTabValue(v)} sx={{ mb: 3, borderBottom: 1, borderColor: 'divider' }}>
        <Tab label={`DOWN LIST (${devices.filter(d => d.status !== "UP").length})`} sx={{fontWeight: 'bold', color: tabValue === 0 ? 'red' : 'inherit'}} />
        <Tab label={`UP LIST (${devices.filter(d => d.status === "UP").length})`} sx={{fontWeight: 'bold', color: tabValue === 1 ? 'green' : 'inherit'}} />
      ...
      {tabValue < 2 ? (
        <Box sx={{ display: 'flex', flexWrap: 'wrap', gap: 2 }}>
          {filteredDevices.map(d => (
            <DeviceBlock key={d.id} device={d} onEdit={() => {
                setEditingId(d.id);
                setFormData({ hostname: d.hostname, ip_address: d.ip_address, snmp_community: d.snmp_community, options: d.options });
                setOpenDialog(true);
        ...
        <Box sx={{ maxWidth: '800px' }}>
          {recentLogs.length === 0 ? (
            <Typography sx={{fontStyle: 'italic', color: 'gray', ml: 2}}>No status changes in the last 10 minutes.</Typography>
          ) : (
            recentLogs.map(log => (
              <Paper key={log.id} sx={{ p: 1.5, mb: 1, display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderLeft: `5px solid ${log.to === 'UP' ? '#10b981' : '#dc2626'}` }}>
                <Box>
                  <Typography variant="body2" fontWeight="bold">{log.hostname} ({log.ip})</Typography>
                  <Typography variant="caption" color="textSecondary">
                    Status changed from <b style={{color: log.from === 'UP' ? 'green' : 'red'}}>{log.from}</b> to <b style={{color: log.to === 'UP' ? 'green' : 'red'}}>{log.to}</b>
                  </Typography>
                </Box>
      ...
      <Dialog open={openDialog} onClose={() => setOpenDialog(false)} fullWidth maxWidth="xs">
        ...
        <DialogContent>
            <TextField label="Hostname" fullWidth margin="dense" value={formData.hostname} onChange={e => setFormData({...formData, hostname: e.target.value})} />
            <TextField label="IP Address" fullWidth margin="dense" value={formData.ip_address} onChange={e => setFormData({...formData, ip_address: e.target.value})} />
            <TextField label="SNMP Community" fullWidth margin="dense" value={formData.snmp_community} onChange={e => setFormData({...formData, snmp_community: e.target.value})} />
        ...
        <DialogActions sx={{p: 3}}>
          ...
          <Button onClick={handleSaveDevice} variant="contained" sx={{bgcolor: '#b50000'}}>Save Device</Button>
        </DialogActions>
      </Dialog>
